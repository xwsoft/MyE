...
public class HourWatchScaleLable extends View {
	 ...
	 * 时间刻度圆形轨迹
	 */
	public  RectF _in_watch = new RectF(20,20,166,166);
	...
	private int _cooling = 0; 
	private int _heating = 0; 
	...
	public HourWatchScaleLable(Context context , int index , int color,int iHour48,int cooling,int heating) {
		super(context);
		_index = index;
		_color = color;
		_iHour48 = iHour48;
	...
	protected void onDraw(Canvas canvas){
	   ...
	   //FontMetrics对象  
         ...
         int width = 45;
         int height = 22;
         float _x = x-space_txt;
         float _y = baseY+5;
	     if(getAngle(_index)>0 && getAngle(_index)<180){
	    	 _x += 5;
	     }
         ShapeDrawable dr = new ShapeDrawable(new RoundRectShape(outerR, null,null));
         dr.getPaint().setColor(0xFFFFFFFF);
         dr.setBounds((int)(_x-2), (int)(_y-15),(int)(_x + width), (int)(_y+4));
	     dr.draw(canvas);
	     dr.getPaint().setStyle(Style.STROKE);
	     dr.getPaint().setStrokeWidth(1);
	     dr.getPaint().setColor(0xFF9F9F9F);
	     dr.draw(canvas);
    	canvas.drawText(_cooling+"", _x,_y, textPaint);
    	textPaint.setColor(0xFF000000);
    	canvas.drawText("/", _x+18,_y, textPaint);
    	textPaint.setColor(0xFF0000FF);
    	canvas.drawText(""+_heating, _x+25,_y, textPaint);
		currentPath = myPath;
		canvas.restore();
	}
}
